...
void DebugUtil::DrawPxShape(UWorld* World, physx::PxRigidActor* PxActor, physx::PxShape* Shape, FColor Color)
{
	...
	DrawPxShape(World, Shape, P2UVector(ShapeGlobalPose.p), P2UQuat(ShapeGlobalPose.q), Color);
}

void DebugUtil::DrawPxShape(UWorld* World, physx::PxShape* Shape, FVector Position, FQuat Rotation, FColor Color)
{
	...
	if (GeometryType == physx::PxGeometryType::Enum::eCAPSULE) {
		...
		DrawDebugCapsule(World, Position, CapsuleGeometry.halfHeight * 2.0f, CapsuleGeometry.radius,
			Rotation * FQuat(FVector::RightVector, UKismetMathLibrary::GetPI() / 2.0f), Color, true);
	}
